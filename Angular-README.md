# Reference links for Angular 2/4 projects #

## Deal with form control ##
https://scotch.io/tutorials/how-to-deal-with-different-form-controls-in-angular-2

## Add/use charts in Angular Application ##
 https://scotch.io/tutorials/interactive-angular-2-charts-with-zingchart
 https://www.zingchart.com/docs/chart-types/pie-charts/
 
##  progressive web application with angular ##
https://youtu.be/51Nd_mTpugY

## debugg  angular application ##
 https://youtu.be/gxixM90vo9Y
 
## Deploying an Angular 4 App ##
https://www.youtube.com/watch?v=nxV3weqMzYo

## Angular 4 SEO Tutorial (Universal + Angular CLI) ##
https://www.youtube.com/watch?v=lncsmB5yfzE

## Step by Step Improving Startup Performance with Lazy Loading in Angular - Manfred Steyer ##
https://www.youtube.com/watch?v=8VLYjt81-fE
https://www.softwarearchitekt.at/post/2017/04/06/lazy-loading-with-angular-material-from-ng-conf-2017-in-salt-lake-city.aspx
https://github.com/manfredsteyer/lazy-loading-ng-conf

## Automatic Progressive Web Apps using the Angular Mobile Toolkit Maxim Salnikov ##
https://www.youtube.com/watch?v=ecu1vAO23ZM
